...
- [internal/edgeagent/webshell/agent_shell_test.go](file://internal/edgeagent/webshell/agent_shell_test.go)
- [web/src/pages/DeviceShell.tsx](file://web/src/pages/DeviceShell.tsx)
</cite>

## 更新摘要
**所做更改**
- 更新了隧道协议部分，新增 Agent 模式通信支持
- 添加了 ShellOpenRequest Mode 字段的详细说明
- 增强了响应结构中的 OS 用户信息字段
- 新增了 Agent 模式的完整实现说明和序列图
- 更新了 WebShell 连接流程以支持双模式（SSH/Agent）
...
**更新** 本次更新扩展了隧道协议以支持 Agent 模式通信，包括 ShellOpenRequest 的 Mode 字段和增强的响应结构中的 OS 用户信息。
...
- **新增 Agent 模式终端**：支持边缘端直接启动 PTY shell，无需 OS 凭据，增强安全性和灵活性。
...
- **新增 Agent 模式支持**
  - 支持两种模式：`ssh`（传统模式，需要 OS 凭据）和 `agent`（新代理模式，无需 OS 凭据）。
  - Agent 模式下，边缘端直接启动 PTY shell，运行于边缘进程用户下，无需分布式 OS 密码。
  - 响应中包含 `os_user` 字段，显示边缘端 PTY 运行的操作系统用户。
- 会话生命周期
...
FE->>WS : 首帧 ShellOpen (mode=agent, cols, rows)
alt Agent 模式
WS->>Agent : shell_open (Mode="agent")
Agent->>PTY : 启动 PTY shell
PTY-->>Agent : 输出字节
Agent-->>WS : shell_output
WS-->>FE : 二进制帧(终端输出)
else SSH 模式
WS->>WS : 传统 SSH 转发
end
...
- [internal/edgeagent/webshell/agent_shell.go:93-187](file://internal/edgeagent/webshell/agent_shell.go#L93-L187)
- [web/src/api/webshell.ts:54-69](file://web/src/api/webshell.ts#L54-L69)
- [web/src/pages/DeviceShell.tsx:244-267](file://web/src/pages/DeviceShell.tsx#L244-L267)
...
- **新增 Agent 模式终端消息**
  - shell_open：建立新的终端会话，支持 mode 字段选择后端（"agent" 或 "ssh"）。
  - shell_input：发送 stdin 数据到活跃会话。
  - shell_resize：更新 PTY 窗口大小。
  - shell_close：终止会话。
  - shell_output：边缘端推送 stdout/stderr 数据。
  - shell_exit：终端退出帧，包含退出码和错误信息。
- 边云交互
...
Mgr-->>Tunnel : accepted
Tunnel-->>Edge : 接受数量
Note over Edge,Mgr : Agent 模式终端会话
Edge->>Tunnel : shell_open(mode="agent", session_id, cols, rows)
Tunnel->>Mgr : 分发 shell_open
Mgr->>Biz : 启动 PTY shell
Biz-->>Mgr : ShellOpenResponse (ok=true, os_user)
Mgr-->>Tunnel : 响应
Tunnel-->>Edge : 会话建立成功
Edge->>Tunnel : shell_input(data)
Tunnel->>Mgr : 分发 shell_input
Mgr-->>Tunnel : 确认
Tunnel-->>Edge : 输入处理完成
```
...
- 并发与限流
  ...
  - **Agent 模式会话限制**：每个边缘端最多支持 5 个并发 Agent shell 会话，防止资源耗尽。
- 缓存与去重
...
- **Agent 模式问题**
  - 检查环境变量 `ONGRID_EDGE_AGENT_SHELL_DISABLED` 是否设置为禁用状态。
  - 验证会话数量是否达到上限（默认 5 个）。
  - 确认 PTY 进程是否正确启动和清理。
- 系统健康检查
...
- [internal/edgeagent/webshell/agent_shell.go:88-91](file://internal/edgeagent/webshell/agent_shell.go#L88-L91)

## 结论
Ongrid 以 proto 契约为核心，结合 REST、gRPC、WebSocket 与 SSE 形成多模态通信体系。通过统一的认证与错误处理机制，保障跨接口的稳定性与可观测性。tunnel 协议支撑边云双向通信，满足设备注册、心跳、指标上报与 Kubernetes 运维等场景。**新增的 Agent 模式终端功能**提供了更安全的交互式访问方式，无需分布式 OS 凭据即可访问边缘设备。建议在部署中关注连接池、速率限制与资源监控，以确保高可用与高性能。
...
- **Agent 模式终端会话**
  - 参考：[internal/pkg/tunnel/messages.go:102-135](file://internal/pkg/tunnel/messages.go#L102-L135)
  - 参考：[internal/edgeagent/webshell/agent_shell.go:93-187](file://internal/edgeagent/webshell/agent_shell.go#L93-L187)
  - 参考：[internal/edgeagent/webshell/agent_shell_test.go:97-131](file://internal/edgeagent/webshell/agent_shell_test.go#L97-L131)
- WebShell 连接与退出
...
- [internal/pkg/auth/middleware.go:21-67](file://internal/pkg/auth/middleware.go#L21-L67)
- [internal/pkg/auth/jwt.go:73-99](file://internal/pkg/auth/jwt.go#L73-L99)
- [internal/edgeagent/webshell/agent_shell.go:14-15](file://internal/edgeagent/webshell/agent_shell.go#L14-L15)
- [internal/edgeagent/webshell/agent_shell.go:46-49](file://internal/edgeagent/webshell/agent_shell.go#L46-L49)