...
- [PRD-005-webssh-agent-mode.md](file://docs/requirements/PRD-005-webssh-agent-mode.md)
- [agent_shell.go](file://internal/edgeagent/webshell/agent_shell.go)
- [handler.go](file://internal/edgeagent/webshell/handler.go)
- [DeviceShell.tsx](file://web/src/pages/DeviceShell.tsx)
- [router.go](file://internal/manager/biz/webshell/router.go)
...
## 更新摘要
**变更内容**
- 新增 WebSSH 代理模式（Agent PTY）功能，支持无需 OS 凭据的直接终端访问
- 更新了边缘代理的 WebSSH 实现，增加了 agent_shell.go 模块
- 增强了前端 DeviceShell 页面，支持模式选择和用户界面优化
- 完善了管理器端的 WebSSH 处理器，支持双模式处理
- 更新了安全模型和配置选项
...
**更新** 新增了 WebSSH 代理模式功能，允许在无法分发 SSH 凭据的环境中直接访问边缘设备的终端，提升了用户体验。
...
WS["WebSSH 处理器<br/>server/webshell"]
end
subgraph "边缘代理"
E["边缘主进程<br/>cmd/ongrid-edge/main.go"]
A["Agent 生命周期<br/>biz/agent.go"]
S["命令沙箱<br/>cmdpolicy/*"]
W["WebSSH 代理<br/>webshell/agent_shell.go"]
C["采集器/插件"]
end
subgraph "Kubernetes"
...
- [agent_shell.go:1-325](file://internal/edgeagent/webshell/agent_shell.go#L1-L325)
...
- WebSSH 代理模式：新增的 Agent PTY 模式，支持无需 OS 凭据的直接终端访问。
...
- [PRD-005-webssh-agent-mode.md:1-65](file://docs/requirements/PRD-005-webssh-agent-mode.md#L1-L65)
...
**更新** 新增了 WebSSH 代理模式，支持两种连接方式：传统的 SSH 模式和新的 Agent 直连模式。
...
participant WS as "WebSSH 处理器<br/>server/webshell"
...
- [main.go:208-300](file://cmd/ongrid/main.go#L208-L300)
- [usecase.go:294-370](file://internal/manager/biz/topology/usecase.go#L294-L370)
- [http.go:389-465](file://internal/manager/server/webshell/http.go#L389-L465)
...
### WebSSH 代理模式（新增功能）
**新增** WebSSH 代理模式提供了无需 OS 凭据的直接终端访问能力，解决了企业环境中无法分发 SSH 密码的问题。

- **模式选择**：前端提供 Agent 直连和 SSH 账号密码两种模式，默认选择 Agent 直连
- **PTY 管理**：边缘代理直接以进程身份（通常为 root）拉起 PTY shell，通过隧道 RPC 双向传输
- **安全模型**：复用平台账号体系（casbin device:shell），审计记录在管理器侧
- **主机开关**：支持 `ONGRID_EDGE_AGENT_SHELL_DISABLED=true` 环境变量控制
- **会话限制**：每主机会话上限 5 个，防止资源滥用

```mermaid
sequenceDiagram
participant Browser as "浏览器"
participant Manager as "管理器 WebSSH"
participant Edge as "边缘代理"
participant PTY as "PTY Shell"
Browser->>Manager : WebSocket 连接
Manager->>Edge : shell_open (Mode="agent")
Edge->>Edge : 检查主机开关与会话限制
Edge->>PTY : 启动 PTY shell
PTY->>Edge : 输出数据
Edge->>Manager : shell_output
Manager->>Browser : 二进制帧输出
Browser->>Manager : 输入数据
Manager->>Edge : shell_input
Edge->>PTY : 写入输入
PTY->>Edge : 退出信号
Edge->>Manager : shell_exit
Manager->>Browser : 关闭连接
```

图表来源
- [PRD-005-webssh-agent-mode.md:20-29](file://docs/requirements/PRD-005-webssh-agent-mode.md#L20-L29)
- [agent_shell.go:93-135](file://internal/edgeagent/webshell/agent_shell.go#L93-L135)
- [http.go:389-465](file://internal/manager/server/webshell/http.go#L389-L465)
- [DeviceShell.tsx:515-705](file://web/src/pages/DeviceShell.tsx#L515-L705)
...
- 边缘代理环境变量（部分）：
  ...
  - **新增** ONGRID_EDGE_AGENT_SHELL_DISABLED：Agent 直连模式开关
...
- 管理操作：
  ...
  - **新增** WebSSH 模式：支持 Agent 直连和 SSH 账号密码两种方式
...
- [edge_main.go:537-668](file://cmd/ongrid-edge/main.go#L537-L668)
- [PRD-005-webssh-agent-mode.md:31-39](file://docs/requirements/PRD-005-webssh-agent-mode.md#L31-L39)
...
- **新增** WebSSH 模块依赖隧道 RPC、WebSocket 桥接、会话路由等功能。
...
- [edge_main.go:66-150](file://cmd/ongrid-edge/main.go#L66-L150)
- [agent.go:186-271](file://internal/edgeagent/biz/agent.go#L186-L271)
- [router.go:1-212](file://internal/manager/biz/webshell/router.go#L1-L212)
...
- **新增** WebSSH 会话限制：每用户最多 5 个并发会话，每设备最多 5 个会话，空闲超时 15 分钟自动关闭。

章节来源
- [agent.go:608-693](file://internal/edgeagent/biz/agent.go#L608-L693)
- [agent.go:273-321](file://internal/edgeagent/biz/agent.go#L273-L321)
- [policy.go:26-31](file://internal/edgeagent/cmdpolicy/policy.go#L26-L31)
- [http.go:105-114](file://internal/manager/server/webshell/http.go#L105-L114)
...
- **新增** WebSSH 连接失败：检查 edge 版本是否支持 Agent 直连；确认主机开关未禁用；查看会话限制是否达到上限。

章节来源
- [agent.go:530-602](file://internal/edgeagent/biz/agent.go#L530-L602)
- [policy.go:707-800](file://internal/edgeagent/cmdpolicy/policy.go#L707-L800)
- [edge_main.go:537-668](file://cmd/ongrid-edge/main.go#L537-L668)
- [http.go:411-423](file://internal/manager/server/webshell/http.go#L411-L423)

## 结论
本系统通过清晰的分层与职责划分，实现了边缘代理的全生命周期管理、Kubernetes 集群集成、网络拓扑构建与安全远程执行。管理器与边缘通过隧道解耦，具备高可用与可扩展性；策略与沙箱保障远程操作的安全性；拓扑与设备模型支撑大规模异构资源管理。新增的 WebSSH 代理模式进一步提升了用户体验，特别是在无法分发 SSH 凭据的企业环境中。建议在生产环境中合理配置网络发现、指标推送与命令策略，结合前端界面进行集群与升级管理，持续优化性能与可观测性。
...
- 边缘代理环境变量示例：
  ...
  - **新增** ONGRID_EDGE_AGENT_SHELL_DISABLED=false（默认启用 Agent 直连）
...
- 管理操作：
  ...
  - 集群入网：通过 bootstrap token 调用 /internal/k8s/enroll
  - 升级管理：选择升级包，分阶段下发 apply_package，健康标记决定回滚
  - **新增** WebSSH 连接：选择 Agent 直连模式无需凭据，或选择 SSH 模式输入账号密码
...
- [PRD-005-webssh-agent-mode.md:41-58](file://docs/requirements/PRD-005-webssh-agent-mode.md#L41-L58)