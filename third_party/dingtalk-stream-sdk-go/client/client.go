	...
	go func() {
		for {
			...
			if err != nil {
				...
				safeStructSend(closeChan)
				return
			}
			if messageType == websocket.TextMessage {
				safeBytesSend(readChan, message)
			}
		}
	}()
...
// safeStructSend / safeBytesSend 是本地补丁（上游 v0.9.1 竞态修复）：
// processLoop 退出时按 defer 顺序 close(readChan/pongChan/closeChan)，
// 但读协程、pong handler、ping 超时协程是独立协程，断连瞬间向已关闭
// 通道发送会触发 "send on closed channel" panic 且无人 recover，直接
// 带崩宿主进程（2026-08-31 ongrid manager 崩溃实证）。发送改走这里
// 就地 recover，丢弃一次已无接收方的信号——语义与原发送一致（通道
// 关闭本身就代表"该退出了"）。上游修复后可移除本副本与 go.mod replace。
func safeStructSend(ch chan struct{}) {
	defer func() {
		if r := recover(); r != nil {
			logger.GetLogger().Errorf("safeStructSend: dropped send on closed channel: %v", r)
		}
	}()
	ch <- struct{}{}
}

func safeBytesSend(ch chan []byte, v []byte) {
	defer func() {
		if r := recover(); r != nil {
			logger.GetLogger().Errorf("safeBytesSend: dropped send on closed channel: %v", r)
		}
	}()
	ch <- v
}
